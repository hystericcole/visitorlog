...
extension CGPoint {
	...
	public static let v = CGPoint(x:0, y:1)
	public static let h = CGPoint(x:1, y:0)
...
extension CGSize {
	...
	public func inset(by inset:UIEdgeInsets) -> CGSize {
		return CGSize(width:max(0, width - inset.left - inset.right), height:max(0, height - inset.top - inset.bottom))
	}
	
	public func padded(by inset:UIEdgeInsets) -> CGSize {
		return CGSize(width:max(0, width + inset.left + inset.right), height:max(0, height + inset.top + inset.bottom))
	}
